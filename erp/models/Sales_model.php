...
class Sales_model extends CI_Model
{
	...
	public function getProductNoteSaleItem($sale_id)
    {
        $this->db->select('sale_items.product_noted as note');
		$this->db->join('product_note', 'product_note.name = sale_items.product_noted', 'left');
		$this->db->group_by('sale_items.product_noted');
		$this->db->order_by('product_note.id', 'ASC');
	...
	public function getItemQuantitySaleItems($id){
        $this->db->select('products.name as product_name, products.name_kh as product_name_kh, products.details as product_detail, SUM(COALESCE(erp_sale_items.quantity, 0)) as qty');
