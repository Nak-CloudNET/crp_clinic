...
<div id="wrapper">
    ...
    <div id="receipt-data">
        <div class="text-center">
			...
			<?php } ?>
			<?php
			echo "<h3 style='font-family : Khmer OS Muol !important;'>" . $biller->cf1 . "</h3>" . "<br/>" ;
            echo "<p style='margin-top: -11px; font-size : 14px;'>" . $biller->address . "<br/>";
			...
			<div style="font-size:11px;">
				<table style="width: 100%;">
					<tbody style="font-size: 12px;">
						...
						<tr style="text-align:center;font-size:12px !important;">
							<td style="vertical-align:middle;text-align:left;padding:3px;">អត្រា </td>
							<td style="vertical-align:middle;text-align:left;">$1 = <?= $inv->other_cur_paid_rate. '  ៛'; ?></td>
							<td style="vertical-align:middle;text-align:left;">ថ្ងៃខែឆ្នំា</td>
						...
						<tr style="text-align:center;font-size:12px !important;">
							...
							<td style="vertical-align:middle;text-align:left;">វិក្កយបត្រ</td>
            ...
            <table class="table-condensed receipt no_border_btm" style="width:100%;">
				<thead class="th_font">
					<tr style="border:1px dotted black !important;​">
						<th class="th_font">ល.រ<br/>Nº</th>
						<th class="th_font">ទំនិញ <br/><?= lang("description"); ?></th>						
						<th class="th_font" style="text-align:center;">ចំនួន<br/><?= lang("qty"); ?></th>
						<th class="th_font" style="text-align:right;">តំលៃ<br/> <?= lang("price"); ?></th>
						<?php if ($inv->product_discount != 0) {  ?>
							<th class="th_font"​​​ style="text-align:right;">បញ្ចុះតំលៃ<br/><?= lang("discount"); ?></th>
						<?php } ?>
						<th class="th_font" style="padding-left:10px;padding-right:10px;text-align:right;">សរុប <br/><?= lang("amount"); ?> </th>
					</tr>
				</thead>
                <tbody style="border-bottom:2px solid black; font-size:12px !important;">
                <?php
					$r = 1;
					$m_us = 0;
					$total_quantity = 0;
					...
					foreach ($rows as $row) {
						...
						echo '	<td class="text-left">' . product_name($row->product_name) . ($row->name_kh ? ' (' . $row->name_kh . ')' : '') . ($row->product_noted ? ' <br/>(' . $row->product_noted . ')' : '') . '</td>';
			...
			<table style="width: 100%; margin-top: 5px;">
				...
				<tr style="font-size:13px !important;">
					...
					<?php echo $this->erp->formatMoney($inv->grand_total);?>
			...
			<table class="received" style="width:100%;margin-top: 5px;">
				...
				?>
					<tr>
                        <th style="border-left:2px solid #000;border-top:2px solid #000;border-right:none;padding-right: 12px;width:64%;font-size:13px !important;"  class="text-right">Received (<?= $default_currency->code; ?>) :</th>
                        <th style="font-size:13px !important;border-right:2px solid #000;border-top:2px solid #000;border-left:none;" class="text-right">$ <?= $this->erp->formatMoney($inv->recieve_usd); ?></th>
                    </tr>
                    <tr>
                        <th style="font-size:13px !important;border-left:2px solid #000;border-bottom:2px solid #000;border-right:none;padding-right: 12px;width:64%;"  class="text-right">Received (Riel) :</th>
                        <th style="font-size:13px !important;border-bottom:2px solid #000;border-right:2px solid #000;border-left:none;" class="text-right">៛ <?= $this->erp->formatMoney($inv->recieve_real) ; ?></th>
                    </tr>
				...
				?>
					...
					<tr>
                        <th style="font-size:13px !important;border-top:2px dotted #000;padding-right: 12px;" class="text-right"><?= lang("change_amount_us"); ?>:</th>
                        <th style="font-size:13px !important;border-top:2px dotted #000" class="text-right">
						<?php
                    ...
                    <tr>
                        <th style="font-size:13px !important;border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("change_amount_kh"); ?></th>
						<th style="font-size:13px !important;border-top:2px dotted #000" class="text-right"><?= number_format(round((($pay+$pay_kh) - $grand_totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate)), -3) . '៛' ; ?></th>
                    </tr>
				<?php
						}
					}else{
                    ...
                    <tr>
                        <th style="font-size:13px !important;border-top:2px dotted #000;padding-right: 12px;" class="text-right"><?= lang("change_amount_kh"); ?></th>
						<th style="font-size:13px !important;border-top:2px dotted #000" class="text-right">
				...
				?>
					<table style="width:100%;">
						...
						?>
						<tr>
							<th style="font-size:13px !important; border:2px solid #000;border-right:none;padding-right: 12px;width:81%;" colspan="<?= $colspan ?>" class="text-right">Received (Riel):</th>
							<th style="font-size:13px !important; border:2px solid #000;border-left:none;" class="text-right"><?= $inv->other_cur_paid . '  ៛' ; ?></th>
						</tr>
						<?php
							}else{}
						?>
				...
				?>
					<tr>
                        <th style="font-size:13px !important;border:2px solid #000;border-right:none;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">Received (<?= $default_currency->code; ?>):</th>
                        <th style="font-size:13px !important; border:2px solid #000;border-left:none;" class="text-right"><?= $this->erp->formatMoney($inv->paid); ?></th>
                    </tr>
					<?php
						if($inv->other_cur_paid){
					?>
                    <tr>
                        <th style="font-size:13px !important; border:2px solid #000;border-right:none;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">Received (Riel):</th>
                        <th style="font-size:13px !important; border:2px solid #000;border-left:none;" class="text-right"><?= $inv->other_cur_paid . '  ៛' ; ?></th>
                    </tr>
					<?php
						}else{}
				...
				?>
					<tr>
                        <th style="font-size:13px !important; border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_us"); ?></th>
                        <th style="font-size:13px !important; border-top:2px dotted #000" class="text-right">
						<?php
                    ...
                    <tr>
                        <th style="font-size:13px !important; border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_kh"); ?></th>
                        <th style="font-size:13px !important; border-top:2px dotted #000" class="text-right"><?= number_format(abs((($pay+$money_kh) - $grand_totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate))) . ' ៛' ; ?></th>
                    </tr>
				<?php
						}
					}else{
                    ...
                    <tr>
                        <th style="font-size:13px !important; border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_kh"); ?></th>
                        <th style="font-size:13px !important; border-top:2px dotted #000" class="text-right"><?= number_format(abs((($pos_paid+$amount_kh_to_us) - $grand_totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate))) . ' ៛' ; ?></th>
                    </tr>
...
<?php } ?>	
	...
	<span class="pull-right col-xs-12">
        <a href="<?= site_url('sales/invoice_clinic/'.$inv->id); ?>" target="_blank" class="btn btn-block btn-primary"><?= lang("print_a4"); ?></a>
    </span>
    <?php } ?>
